...
- [RC-GRPO](https://arxiv.org/abs/2602.03025) names the failure mode we watch for in class: SFT -> GRPO can stall when each group has all-success or all-failure rewards, because the normalized advantage carries little information. The course addresses the first version of this problem with dynamic no-signal group filtering and diagnostic metrics; advanced variants can add explicit exploration-conditioning or mixed-difficulty curricula.
...
- [AutoForge](https://huggingface.co/papers/2512.22857) treats environment synthesis and simulator stability as first-class parts of agentic RL. The course keeps the first lab on a fixed retail replay environment, then points advanced users to the official tau2 runtime when they need realistic user/environment variation.
- [CM2](https://huggingface.co/papers/2602.12268) uses checklist-style rewards for multi-turn tool agents when a fully verifiable final state is not always available. In this course, that idea maps to RULER/checklist rewards as an auxiliary or comparison branch, not as a replacement for DB/state-action rewards when those are available.
- [ET-Agent](https://huggingface.co/papers/2601.06860) and related progressive reward-shaping work reinforce the same lesson as IRC: tool-agent RL usually needs behavior calibration, not merely more steps. Track train-step reward, validation outcome, state-changing-action errors, and checkpoint candidates together.
