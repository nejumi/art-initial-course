...
async def main_async() -> None:
    ...
    cfg = config_from_env()
    if args.weave:
        ensure_wandb_run(cfg, job_type="gspo")
        init_weave()
    ...
    if len(scenarios) < minimum_scenarios:
        raise ValueError(
            "GSPO requires at least one unique scenario per trajectory group: "
    ...
    for _ in range(args.steps):
        ...
        if not groups:
            ...
            log_wandb_metrics(cfg, dynamic_filter_metrics, job_type="gspo", step=last_step)
            append_step_metrics(
                args.metrics_jsonl,
                step=last_step,
                algorithm="gspo",
        ...
        log_wandb_metrics(cfg, metrics, job_type="gspo", step=result.step)
        last_step = result.step
        append_step_metrics(args.metrics_jsonl, step=result.step, algorithm="gspo", metrics=metrics)
